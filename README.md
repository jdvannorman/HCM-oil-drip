...
7. To reuse the shape later, select the area, click **Save shape**, give it a name, then choose that name and click **Use** in a later session. Presets are stored in `area_presets.json`.
8. **Add another area / 新增區域** creates any additional named contour. Areas are not limited to Left or Right.
9. For a fixed manual rectangle, use **Manual rectangle**, which switches to manual mode.

Status meanings:

- **Tracking / 邊緣已定位:** current edges passed the geometric, color, and texture checks.
- **No material / 無卷材:** both coil-edge appearance checks failed. Defect detection is paused and no event is produced.
- **Material found / 確認卷材中:** material has reappeared but is being confirmed before background calibration and counting resume.
...
Each automatic result includes `region_timeline.json` and a `tracking` summary in `results.json`, recording usable, no-material, material-confirming, held, lost, and recalibrating frames. Older results remain readable and retain their original manual regions. Automatic mode adds processing cost; this version is intended for offline video analysis and does not guarantee 30 fps throughput.

Oil-event detection still runs on every decoded frame while material is present, so a fast droplet is not skipped. Material presence is checked independently once per second using the expected left/right coil-edge contrast and texture. If the coil has not entered yet or has already left, frames are recorded as **No material** and defect counting stops. Reappearance must be confirmed and the detector background is rebuilt before counting resumes. A video may therefore begin before rolling starts; calibration searches forward for the first reliable material interval instead of failing on the opening second.

Area geometry uses an adaptive schedule: it starts with a full edge check every five frames, and after four consistent checks it drops to a once-per-second safety check for the remainder of startup. It then checks small camera motion and material presence once per second and performs the selected full geometry recheck every 1, 5, or 10 minutes. A detected camera movement immediately forces a new full edge check. Failed or unsafe geometry checks enter Held/Lost and pause counting as before. The default stable interval is 10 minutes.

The supplied original videos are approximately 30 fps (the marked `_C` copies are approximately 30.3 fps). The detector overlaps decoding of the next frame with processing of the current frame; it does not skip frames. On the i5-14500T, the optimized Fast mode measured about 39–54 detection fps on representative 1920×1080 samples, or roughly 1.3–1.8× real-time. The result screen records both detection fps and its real-time ratio. MP4 clips are only made later when the user requests an event export.

**CPU usage / CPU 使用量** controls OpenCV worker threads. **Auto-select / 自動選擇** is the default. On the first automatic-area scan, the program measures the actual video decode, edge tracking, resizing, and detector workload twice with 7, 12, and 18 threads, chooses the fastest result, and saves it in `cpu_tuning.json`. The one-time check can take about 10 seconds; later scans reuse it. Balanced, Fast, and Maximum remain available as manual overrides. These are processing-thread limits, not guaranteed Task Manager percentages; decoding, disk speed, operation size, and Windows scheduling affect actual utilization. More threads are not necessarily faster.

The three monitored regions keep independent backgrounds and event counts. Their slowly changing polygon masks and morphology kernels are reused instead of being rebuilt on every frame. Two larger "single combined image" implementations were rejected during validation because their memory-copy cost made real scans slower, even though their detections matched.
...
2. Select an event in the results table. The player jumps to its peak and zooms into the relevant area. The strongest detected location is circled in orange and labelled **DETECTED**, so it can be judged as a real drip or as a reflection that should be excluded by editing the contour.
...
5. Scanning does not create MP4 files. Select an event, choose an **Export speed** such as `0.10x`, `0.25x`, `0.50x`, or `1.00x`, then choose **Export event video / 匯出影片** only when a full and close-up clip is needed. The default is `0.25x`. You may also type a custom value from `0.05x` to `2.00x`, or a percentage such as `25%`. After export, **Play event clip** opens the selected-speed close-up MP4.

Slow export keeps every source frame and lowers only the output playback frame rate. It does not change detection results, source timestamps, or the original MP4. Different speeds are saved side by side with suffixes such as `_0p25x`; exporting one speed does not overwrite another speed.
...
During a scan, **Pause scan / 暫停掃描** stops after the current frame without discarding completed progress. The event table immediately shows a provisional preview of candidates found in the scanned portion, together with the scanned time/frame boundary; the unscanned remainder is explicitly not included. These provisional records can be opened in the source viewer, but review decisions and exports remain locked until the scan finishes because events can merge or change after resuming. The button changes to **Resume scan / 繼續掃描**. **Cancel** ends the run; any already completed video results remain saved.

### Same-location groups and timing / 同位置群組與時間規律

The result table defaults to **Location groups / 位置分組**. Three or more candidates that recur near the same source-image position are collapsed into **位置群組 1, 2, 3…**, regardless of whether their timing is stable. This is useful when lighting, missed detections, or changing speed makes one physical reflection appear at irregular times. Groups are numbered by occurrence count, so the most frequently repeated location is shown first. Expand a group to inspect every original event.

The second selector above the table filters directly to **位置群組 1, 2, 3…**, all locations, or ungrouped events. **All individual events / 全部事件** removes the visual nesting while retaining the selected location filter. **Timing regular only / 只看時間規律** shows the stricter time-pattern analysis.

The third selector filters the conservative visual assessment:

- **Likely reflection jitter / 疑似反光抖動:** many short events recur in a compact position with very little spatial spread. This is a review hint, never an automatic rejection; a fixed oil outlet can look similar.
- **Possible moving droplet / 疑似移動油滴:** a newly scanned event has measurable downward movement across its positive frames. It still requires visual review because moving glare, smoke, and vibration can imitate motion.
- **Uncertain / 無法確定:** the evidence is insufficient for either label.

Older saved results can be assessed for compact repeated reflection jitter when opened. They do not contain the newly added first/last within-event positions, so they are not retroactively labelled as moving droplets. A new scan is required for the downward-motion feature.

Location and timing are separate fields. A location group says only that events recur at a similar image position; it does not claim a fixed frequency. Within that group, the timing column can still report a fixed interval, a rising/falling frequency, or **No timing pattern / 無時間規律**.

For the stricter timing analysis, candidates that recur in the same monitored area at least four times within a 60-second window with stable intervals receive an `R…` timing-pattern ID. The timing row shows its time span, estimated interval, and occurrences per minute.

Changing machine speed is supported. A sequence with at least five occurrences can also be grouped when its intervals change smoothly rather than staying fixed. For example, intervals of 8, 7, 6, and 5 seconds are shown as `8→5s · frequency rising / 頻率上升`; the reverse is shown as **frequency falling / 頻率下降**. The trend must be smooth, remain at the same location, and fit inside the same 60-second review window. Random or abrupt timing changes remain unclassified.

This is a review classification, not an automatic rejection. A trimmer/chopper reflection synchronized to the machine is likely to be regular, but a genuinely periodic oil drip can also be regular. The strongest event in a group is shown first for visual review, and every underlying event remains available for accepting, rejecting, circling, or exporting. Existing result folders are classified when opened, so a rescan is not required just to use the new view.
...
- **Location group:** at least three candidates near one source-image location, independent of timing. This can collect a reflection or defect whose apparent frequency changes substantially.
- **Regular timing pattern:** at least four candidates near one source-image location with sufficiently stable timing inside a one-minute window, or at least five whose frequency rises or falls smoothly. It is not automatically rejected.
- **No timing pattern:** no sufficiently stable frequency sequence was established. The event can still belong to a same-location group and this does not prove it is an oil drip.
- **Likely reflection jitter:** compact, frequent, mostly one-to-three-frame activity at one repeated location. It remains available for human review and is not automatically rejected.
- **Possible moving droplet:** downward within-event motion was measured in a newly scanned result. It is a candidate, not a certified oil drip.
- **Uncertain:** reflection, fluid, smoke, or another process change could not be separated reliably.
...
- **Save shape / Use:** remember a left, center, right, or custom contour by name and reapply it in a future session. Automatic presets remain strip-relative, so they continue to follow strip width and camera movement.
...
- **CPU usage:** Auto-select benchmarks this computer once and is recommended. Balanced, Fast, and Maximum are manual overrides; Maximum is not guaranteed to be fastest.
...
| `events.csv` | Same event list with location group, timing fields, visual classification, within-event motion, and review decisions |
| `location_groups.csv` | One row per repeated location, including occurrence count, position spread, reflection-jitter score, visual classification, member IDs, and timing summary |
| `regular_patterns.csv` | One row per regular group: area, occurrence count, time range, starting/ending interval, frequency trend, regularity score, center, and member event IDs |
| `clips/E…[_0p25x].mp4` | On-request full-frame event clip; slow exports include a speed suffix |
| `clips/E…[_0p25x]_closeup.mp4` | On-request 2× enlarged crop at the selected playback speed |
| `clips/E…[_0p25x].jpg` | On-request peak-frame evidence, highlighting the strongest detected component |

The JPEG box identifies the strongest component, not every splash fragment. Enlarging a crop helps review but does not recover missing optical detail. Clips use MPEG-4 Part 2 video in MP4 containers and have no audio. At `1.00x` they retain approximately the original frame rate; slow exports retain every frame at a proportionally lower output frame rate. They are re-encoded copies; the source remains available for comparison.

...
- **Regular pattern:** at least four candidates near one source-image location with sufficiently stable timing inside a one-minute window. It is separated for review as a possible synchronized reflection or periodic process defect; it is not automatically rejected.
...
| `events.csv` | Same event list with review decisions plus `pattern_id`, `pattern_class`, and interval columns, readable in Excel |
| `regular_patterns.csv` | One row per regular group: area, occurrence count, time range, interval, frequency, regularity score, center, and member event IDs |

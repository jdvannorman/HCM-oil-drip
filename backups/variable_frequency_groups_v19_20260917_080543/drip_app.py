...
from drip_core import (ROOT,DEFAULT_CONFIG,LOGICAL_CPUS,DEFAULT_CPU_THREADS,analyze,open_video,
                       pixel_box,save_json,write_csv,write_patterns_csv,Cancelled,export_event,classify_regular_patterns)
import cv2
import numpy as np
...
class App:
    ...
    def build(self):
        ...
        self.event_view=tk.StringVar(value='Grouped review / 規律分組')
        view=ttk.Combobox(results_head,textvariable=self.event_view,
                          values=['Grouped review / 規律分組','All individual events / 全部事件','Regular patterns only / 只看規律'],
                          state='readonly',width=28)
        view.pack(side='right',padx=(0,10))
        ...
        for key,title,width in [('time','Video time (s)',105),('side','Area',65),('kind','Type',125),('pattern','Regularity / 規律性',180),('state','Review status',105),('count','Confirmed droplets',125)]:
    ...
    def set_report(self,folder,report):
        ...
        if report.get('state')=='paused_preview':
            ...
            self.warning.set(f'PAUSED PREVIEW • {len(report["events"])} provisional candidates • {len(patterns)} regular groups • scanned {seconds:.1f}s ({frames}/{total} frames). Unscanned video is not included.')
            return
        ...
        self.warning.set(f"{len(report['events'])} candidates • {len(patterns)} regular groups ({regular} events) • Detection {speed} • {cpu_note} • Human review required. "
    ...
    def refresh_events(self,selected=None):
        ...
        def add_event(e,parent=''):
            ...
            pattern=e.get('pattern_id') or 'Not regular / 無明顯規律'
        ...
        def add_pattern(pattern):
            ...
            label=f"{pattern['id']} ({pattern['occurrences']}×)"
            timing=f"{pattern['interval_seconds']:.3g}s interval · {pattern['frequency_per_minute']:.2g}/min"
        ...
        mode=self.event_view.get() if hasattr(self,'event_view') else 'Grouped review / 規律分組'
        if mode.startswith('All'):
            for e in events:add_event(e)
        elif mode.startswith('Regular'):
            for pattern in patterns:add_pattern(pattern)
        else:
            entries=[(e['peak_seconds'],'event',e) for e in events if e['id'] not in regular_ids]
            entries += [(p['first_seconds'],'pattern',p) for p in patterns]
            for _,kind,item in sorted(entries,key=lambda row:row[0]):
                add_pattern(item) if kind=='pattern' else add_event(item)
        ...
        self.counts.set(f'{len(events)} raw  |  {len(patterns)} regular groups ({regular_count} events)  |  {irregular_count} non-regular  |  {label}')
    ...
    def choose_event(self,event=None):
        ...
        if pattern:
            self.status.set(f"{pattern['id']}: {pattern['occurrences']} occurrences near one location, about every {pattern['interval_seconds']:.3g}s. Possible synchronized reflection or periodic defect; review required.")
    ...
    def persist(self,e):
        ...
        self.status.set('Review saved to results.json, events.csv, and regular_patterns.csv.')
    ...
    def mark(self,state):
        ...
        if self.selected_pattern():
            self.status.set('Expand the regular group and select an individual event before accepting or rejecting it.')
            return
        if self.report.get('state')=='paused_preview':
    ...
    def save_details(self):
        ...
        if not e:return
        if self.selected_pattern():
            self.status.set('Expand the regular group and select an individual event before saving review details.')
            return
        if self.report.get('state')=='paused_preview':

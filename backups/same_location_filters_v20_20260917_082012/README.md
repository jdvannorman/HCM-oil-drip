...
### Regular-location/frequency review / 同位置規律事件

The result table defaults to **Grouped review / 規律分組**. Candidates that recur in the same monitored area, within approximately 18–52 source pixels, at least four times within a 60-second window, and with stable intervals are collapsed under one `R…` row. The row shows its time span, estimated interval, and occurrences per minute. Expand it to inspect every original event. Use **All individual events / 全部事件** to remove the visual grouping, or **Regular patterns only / 只看規律** to inspect only these groups.
...
- **Regular pattern:** at least four candidates near one source-image location with sufficiently stable timing inside a one-minute window, or at least five whose frequency rises or falls smoothly. It is separated for review as a possible synchronized reflection or periodic process defect; it is not automatically rejected.
- **Not regular:** no sufficiently stable location-and-frequency sequence was established from the available candidates. This does not prove the event is an oil drip.
...
| `events.csv` | Same event list with review decisions plus `pattern_id`, `pattern_class`, interval, and frequency-trend columns, readable in Excel |

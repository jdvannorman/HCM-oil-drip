...
def analyze_auto(path,output_root,config,progress,cancel):
    from drip_core import (ROOT,open_video,check_cancel,save_json,gray_blur,RegionDetector,
    ...
    try:
        ...
        for index in range(meta['frames']):
            ...
            record=dict(frame=index,status=state['status'],quality=state['edges']['quality'] if state['edges'] else 0,regions=regions)
            ...
            for r in regions:
                ...
                spots=detectors[name].detect(crop)
        ...
        report['tracking']=dict(frame_counts=counts,usable_frames=counts.get('tracking',0),total_frames=len(timeline))
        ...
        report.update(state='complete',performance=dict(detection_seconds=round(detection_time,3),analysis_fps=round(meta['frames']/detection_time,2),
                       total_seconds=round(time.perf_counter()-started,3),process_peak_working_set_mb=peak_memory_mb(),alignment_skipped_frames=skipped))
        save_json(folder/'results.json',report)

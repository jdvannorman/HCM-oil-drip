...
import sys
from auto_roi import AreaTracker,regions_from_edges
...
class App:
    ...
    def load_video(self,path,find_report=True):
        ...
        try:
            ...
            self.auto_tracker=AreaTracker(meta['fps'])
            self.auto_state=None
            self.preview_regions=None
            self.area_timeline=None
            self.reviewing=False
    ...
    def seek(self,index):
        ...
        if self.auto_tracker and index!=previous+1:self.auto_tracker=AreaTracker(self.meta['fps'])
        self.update_areas()
    ...
    def current_region(self):
        ...
        return next((r for r in regions if r['name']==self.region.get()),regions[0])
    ...
    def change_area_mode(self):
        ...
        if self.meta:self.auto_tracker=AreaTracker(self.meta['fps'])
        self.update_areas()
        self.render()

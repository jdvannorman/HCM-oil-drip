...
DEFAULT_CONFIG = {
    ...
    'regions': [
        {'name': 'Left', 'box': [0.319, 0.759, 0.440, 0.988],
         'polygon': [[.28,0],[1,0],[1,1],[.02,1]]},
        {'name': 'Right', 'box': [0.619, 0.719, 0.739, 0.954],
         'polygon': [[0,0],[.78,0],[.98,1],[0,1]]},
    ],
    'threshold': 14.0,
    'min_area': 8,
    'max_area': 1800,
...
def validate_config(config):
    ...
    if config.get('auto_area') and any(r['name'] not in ('Left','Right') for r in config['regions']):
        raise ValueError('Automatic areas require Left and/or Right region names.')
    names = []
...
class RegionDetector:
    ...
    def detect(self, crop):
        ...
        mask=cv2.bitwise_and(mask,self.valid)

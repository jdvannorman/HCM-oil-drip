...
class App:
    ...
    def build(self):
        ...
        self.pause_scan_button=ttk.Button(action_row,text='Pause scan / 暫停',command=self.toggle_scan_pause,state='disabled')
        self.pause_scan_button.pack(side='left',fill='x',expand=True)
    ...
    def run(self,all_files):
        ...
        self.pause_scan_button.configure(state='normal',text='Pause scan / 暫停')
        def worker():
            errors=[]
        ...
        try:
            while True:
                ...
                if kind=='progress':
                    ...
                    self.pause_scan_button.configure(state='disabled',text='Pause scan / 暫停')
                    self.run_gate.set()
    ...
    def toggle_scan_pause(self):
        ...
        if self.run_gate.is_set():
            self.run_gate.clear()
            self.pause_scan_button.configure(text='Resume scan / 繼續')
            self.status.set('Scan paused. Completed progress is retained.')
        else:
            self.run_gate.set()
            self.pause_scan_button.configure(text='Pause scan / 暫停')

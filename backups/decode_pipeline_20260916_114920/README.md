...
2. Click **Preview areas / 預覽新區域**. The supplied Left, Center, and Right contours show the areas that will be analyzed.
...
6. **Add another area / 新增區域** creates any additional named contour. Areas are not limited to Left or Right.
7. For a fixed manual rectangle, use **Manual rectangle**, which switches to manual mode.
...
The supplied original videos are approximately 30 fps (the marked `_C` copies are approximately 30.3 fps). On the i5-14500T, the optimized Fast mode measured about 44–54 detection fps on representative 1920×1080 samples, or roughly 1.5–1.8× real-time. The result screen records both detection fps and its real-time ratio. Clip creation is a separate export phase; many event clips can make total wall-clock time longer than the detection pass.
...
- **Restore auto Left / Center / Right:** restore the three supplied strip-relative shapes.

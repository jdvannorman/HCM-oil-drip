...
class App:
    ...
    def set_report(self,folder,report):
        ...
        if report.get('state')=='paused_preview':
            ...
            self.warning.set(f'PAUSED PREVIEW • {len(report["events"])} provisional candidates • scanned {seconds:.1f}s ({frames}/{total} frames). Unscanned video is not included.')
            return
        ...
        self.warning.set(f"{len(report['events'])} candidates • Detection {speed} • {cpu_note} • Human review required. "

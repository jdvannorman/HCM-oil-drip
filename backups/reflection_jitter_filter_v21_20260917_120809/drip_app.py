...
from drip_core import (ROOT,DEFAULT_CONFIG,LOGICAL_CPUS,DEFAULT_CPU_THREADS,analyze,open_video,
                       ...
                       Cancelled,export_event,classify_regular_patterns,classify_location_groups)
import cv2
import numpy as np
...
class App:
    ...
    def build(self):
        ...
        self.group_filter_box=ttk.Combobox(results_head,textvariable=self.group_filter,
                                           values=['All locations / 全部位置'],state='readonly',width=26)
        self.group_filter_box.pack(side='right',padx=(0,6))
    ...
    def set_report(self,folder,report):
        ...
        self.warning.set(f"{len(report['events'])} candidates • {len(groups)} same-location groups ({grouped} events) • {len(patterns)} timing patterns ({regular} events) • Detection {speed} • {cpu_note} • Human review required. "
    ...
    def refresh_events(self,selected=None):
        ...
        def add_location_group(group):
            ...
            self.tree.insert('','end',iid=iid,text=label,open=False,
                             values=(span,group['region'],'Same-location group',timing,'Group review',''))
        ...
        self.counts.set(f'{len(events)} raw  |  {len(groups)} location groups ({grouped_count} events)  |  {len(patterns)} timing patterns ({regular_count} events)  |  {label}')
    ...
    def choose_event(self,event=None):
        ...
        if pattern:
            ...
            self.status.set(f"{location_group['name']}: {location_group['occurrences']} events recur near the same source-image point. Timing may be regular or variable; expand the group to review individual events.")

...
def select_cpu_threads_for_video(path,config,cancel=None,pause_gate=None):
    ...
    from drip_core import (ROOT,LOGICAL_CPUS,DEFAULT_CPU_THREADS,open_video,check_cancel,
    ...
    signature={'logical_cpus':LOGICAL_CPUS,'opencv':cv2.__version__,'algorithm':'actual_auto_pipeline_v2','candidates':candidates}
...
def analyze_auto(path,output_root,config,progress,cancel,pause_gate=None,partial=None):
    ...
    try:
        ...
        def emit_partial(scanned_frame):
            ...
            snapshot.update(state='paused_preview',events=events,region_timeline=None,

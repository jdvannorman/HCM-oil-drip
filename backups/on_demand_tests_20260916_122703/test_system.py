...
from drip_core import DEFAULT_CONFIG, ROOT, analyze, Cancelled, group_events, validate_config, open_video
import cv2
import numpy as np
...
class DetectorTests(unittest.TestCase):
    ...
    def test_static_bright_reflection_and_two_transient_spots(self):
        with tempfile.TemporaryDirectory(dir=ROOT/'inspection') as tmp:
            ...
            cfg=copy.deepcopy(DEFAULT_CONFIG)
            cfg.update(stabilize=False,regions=[dict(name='Test',box=[0,0,1,1])])

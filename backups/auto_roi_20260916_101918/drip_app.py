...
from drip_core import ROOT, DEFAULT_CONFIG, analyze, open_video, pixel_box, save_json, write_csv, Cancelled, export_event
import cv2
import numpy as np
...
class App:
    ...
    def build(self):
        ...
        self.library=tk.Listbox(left,height=8,font=('Segoe UI',9),bg='white',fg='#172b40',
        ...
        self.region_combo=ttk.Combobox(left,textvariable=self.region,values=['Left','Right'],state='readonly')
        self.region_combo.pack(fill='x')
        self.region_combo.bind('<<ComboboxSelected>>',lambda e:self.render())
        ttk.Button(left,text='Draw detection area',command=self.begin_area).pack(fill='x',pady=(5,0))
        ttk.Button(left,text='Restore sample areas',command=self.reset_areas).pack(fill='x',pady=(5,8))
        ...
        ttk.Checkbutton(left,text='Compensate for camera vibration',variable=self.stabilize).pack(anchor='w',pady=8)
    ...
    def current_region(self):
        return next((r for r in self.config['regions'] if r['name']==self.region.get()),self.config['regions'][0])
    ...
    def settings(self):
        ...
        cfg.update(threshold=self.threshold.get(),pre_seconds=self.pre.get(),post_seconds=self.post.get(),
                   stabilize=self.stabilize.get(),export_clips=True)
    ...
    def set_report(self,folder,report):
        ...
        self.warning.set(f"{len(report['events'])} candidates • Analysis {p.get('analysis_fps','?')} fps • Human review required. "
    ...
    def add_manual(self):
        ...
        try:
            ...
            try:export_event(cap,meta,e,box,self.report_dir/'clips',cfg,None)
            finally:cap.release()

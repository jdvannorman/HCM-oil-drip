...
DEFAULT_CONFIG = {
    ...
    'regions': [
        {'name': 'Left', 'box': [0.319, 0.759, 0.440, 0.988],
         ...
         'strip_polygon': [[.04,.72],[.30,.72],[.30,.98],[.04,.98]]},
        {'name': 'Center', 'box': [0.440, 0.720, 0.619, 0.980],
         'polygon': [[0,0],[1,0],[1,1],[0,1]],
         'strip_polygon': [[.34,.72],[.66,.72],[.66,.98],[.34,.98]]},
        {'name': 'Right', 'box': [0.619, 0.719, 0.739, 0.954],
         'polygon': [[0,0],[1,0],[1,1],[0,1]],
         'strip_polygon': [[.70,.72],[.96,.72],[.96,.98],[.70,.98]]},
    ],
    'threshold': 14.0,
    'min_area': 8,
    'max_area': 1800,
...
def validate_config(config):
    ...
    threads=config.get('cpu_threads',DEFAULT_CPU_THREADS)
    if not isinstance(threads,(int,float)) or not math.isfinite(threads) or int(threads)!=threads or not 1<=threads<=LOGICAL_CPUS:
...
def configure_cpu(config):
    threads=int(config.get('cpu_threads',DEFAULT_CPU_THREADS))
...
class RegionDetector:
    ...
    def detect(self, crop, polygon=None):
        ...
        if polygon is not None:
            ...
            valid=cv2.bitwise_and(dynamic,self.texture_valid)
...
def write_csv(folder, events):
    fields=['id','region','kind','start_seconds','peak_seconds','end_seconds','status','confirmed_drips','positive_frames','max_components','peak_score','clip','closeup_clip','notes']
...
def export_event(cap, metadata, event, box, folder, config, cancel, timeline=None):
    fps=metadata['fps']
...
def analyze(path, output_root=None, config=None, progress=None, cancel=None):
    ...
    if config.get('auto_area',False):
        ...
        return analyze_auto(path,output_root,config,progress,cancel)
    ...
    try:
        ...
        for name,items in observations.items():
            events+=group_events(items,metadata['fps'],config['merge_gap_seconds'])
        ...
        report['warnings'].append('Clip audio is omitted. Sources are never modified.')
        ...
        for i,event in enumerate(events,1):
            ...
            export_event(cap,metadata,event,boxes[event['region']],clips,config,cancel)
        report['performance']=dict(detection_seconds=round(detection_elapsed,3),
                                   analysis_fps=round(metadata['frames']/max(.001,detection_elapsed),2),

...
DEFAULT_CONFIG = {
    ...
    'roi_presence_check_seconds': 1.0,
...
def validate_config(config):
    ...
    playback_speed=config.get('export_playback_speed',1.0)
    if not isinstance(playback_speed,(int,float)) or not math.isfinite(playback_speed) or not .05<=playback_speed<=2.0:
        raise ValueError('Export playback speed must be from 0.05x to 2.0x.')
    ...
    for obs in observations:
        if active is None or obs['frame']-active['last_frame']>max_gap:
            active=dict(region=obs['region'],start_frame=obs['frame'],last_frame=obs['frame'],
                        ...
                        first_bbox=obs['bbox'],last_bbox=obs['bbox'],
                        first_transform=obs.get('transform',[[1,0,0],[0,1,0]]),
                        last_transform=obs.get('transform',[[1,0,0],[0,1,0]]))
            result.append(active)
            for key in ('peak_source_bbox','peak_region'):
                if key in obs:active[key]=obs[key]
            if 'peak_source_bbox' in obs:
                active['first_source_bbox']=obs['peak_source_bbox']
                active['last_source_bbox']=obs['peak_source_bbox']
        else:
            active['last_frame']=obs['frame']
            active['positive_frames']+=1
            active['max_components']=max(active['max_components'],obs['components'])
            active['last_bbox']=obs['bbox']
            active['last_transform']=obs.get('transform',[[1,0,0],[0,1,0]])
            if 'peak_source_bbox' in obs:active['last_source_bbox']=obs['peak_source_bbox']
...
def classify_regular_patterns(events, metadata, window_seconds=60.0, min_occurrences=4,
    ...
    minimum_period=max(.10,3/fps)
    def timing_model(times):
        intervals=np.diff(np.array(times,np.float64))
        if len(intervals)<min_occurrences-1:return None
        period=float(np.median(intervals))
        if period<minimum_period or period>window_seconds/(min_occurrences-1):return None
        tolerance=max(2/fps,.16*period)
        deviations=np.abs(intervals-period)
        score=max(0.0,1.0-float(np.mean(deviations))/tolerance)
        if float(deviations.max(initial=0))<=tolerance and score>=minimum_regularity:
            return dict(model='constant',period=period,score=score,
                        interval_start=period,interval_end=period,trend='stable')
        # A changing-speed pattern needs one extra occurrence. Fit a smooth
        # linear trend to consecutive intervals: shorter intervals mean the
        # source is speeding up; longer intervals mean it is slowing down.
        if len(times)<min_occurrences+1:return None
        x=np.arange(len(intervals),dtype=np.float64)
        slope,intercept=np.polyfit(x,intervals,1)
        predicted=intercept+slope*x
        trend_tolerance=max(2/fps,.10*period)
        residuals=np.abs(intervals-predicted)
        trend_score=max(0.0,1.0-float(np.mean(residuals))/trend_tolerance)
        relative_change=abs(float(predicted[-1]-predicted[0]))/max(period,.001)
        if (predicted.min(initial=period)<minimum_period or relative_change<.10 or
                abs(float(slope))>.35*period or
                float(residuals.max(initial=0))>trend_tolerance or
                trend_score<minimum_regularity):return None
        start_interval=float(predicted[0]);end_interval=float(predicted[-1])
        return dict(model='changing',period=period,score=trend_score,
                    interval_start=start_interval,interval_end=end_interval,
                    trend='frequency_increasing' if end_interval<start_interval else 'frequency_decreasing')
    for event in events:
        event['pattern_id']=''
        event['pattern_class']='unclassified'
        event.pop('pattern_interval_seconds',None)
        event.pop('pattern_frequency_trend',None)
    located=[]
    for event in events:
    ...
    for cluster_index,cluster in enumerate(clusters):
        ...
        for start in range(len(items)-min_occurrences+1):
            ...
            for end in range(start+min_occurrences,maximum+1):
                ...
                timing=timing_model(subset)
                if not timing:continue
                candidates.append({'cluster':cluster_index,'indices':set(range(start,end)),
                                   'period':timing['period'],'score':timing['score'],'items':items})
    merged=[]
    ...
    for group in merged:
        ...
        period=round(timing['period'],3)
        start_interval=round(timing['interval_start'],3)
        end_interval=round(timing['interval_end'],3)
        classification=('regular_changing_frequency' if timing['model']=='changing'
                        else 'regular_location_frequency')
        pattern=dict(id=pattern_id,classification=classification,region=pattern_events[0]['region'],
                     ...
                     interval_seconds=period,interval_start_seconds=start_interval,interval_end_seconds=end_interval,
                     frequency_per_minute=round(60/period,2),frequency_trend=timing['trend'],
                     regularity_score=round(timing['score'],3),
                     center_source_pixels=[round(float(np.mean([p[0] for p in centers])),1),round(float(np.mean([p[1] for p in centers])),1)],
                     review_hint='Regular location with stable or smoothly changing timing; possible machine-synchronous reflection or periodic process defect. Human review required.')
        patterns.append(pattern)
        for event in pattern_events:
            event.update(pattern_id=pattern_id,pattern_class='regular',pattern_interval_seconds=period,
                         pattern_frequency_trend=timing['trend'])
    return patterns

def classify_location_groups(events, metadata, min_occurrences=3):
    """Group repeated events by source-image location, independent of timing.

    Timing regularity and location recurrence are deliberately separate: light,
    missed detections, and machine speed can make one physical reflection look
    temporally irregular while its source position remains highly repeatable.
    """
    width,height=float(metadata.get('width') or 1),float(metadata.get('height') or 1)
    radius=max(20.0,min(48.0,math.hypot(width,height)*.018))
    for event in events:
        event['location_group_id']=''
        event['location_group_name']=''
    located=[]
    for event in events:
        box=event.get('peak_source_bbox')
        if box:center=((float(box[0])+float(box[2]))/2,(float(box[1])+float(box[3]))/2)
        elif event.get('bbox'):
            bx0,by0,bx1,by1=event['bbox']
            point=np.array([[(bx0+bx1)/2,(by0+by1)/2,1]],np.float32)
            transform=np.array(event.get('peak_transform',[[1,0,0],[0,1,0]]),np.float32)
        ...
        located.append((event,center))
    clusters=[]
    for event,center in sorted(located,key=lambda item:item[0].get('peak_seconds',0)):
        choices=[cluster for cluster in clusters if cluster['region']==event['region'] and
                 math.hypot(center[0]-cluster['center'][0],center[1]-cluster['center'][1])<=radius]
        if choices:
            cluster=min(choices,key=lambda item:math.hypot(center[0]-item['center'][0],center[1]-item['center'][1]))
            cluster['items'].append((event,center))
            # A recent-center window prevents one long sequence from slowly
            # drifting across and absorbing a neighboring physical location.
            recent=[point for _,point in cluster['items'][-40:]]
            cluster['center']=(float(np.median([p[0] for p in recent])),float(np.median([p[1] for p in recent])))
        else:clusters.append({'region':event['region'],'center':center,'items':[(event,center)]})
    qualified=[cluster for cluster in clusters if len(cluster['items'])>=min_occurrences]
    qualified.sort(key=lambda cluster:(-len(cluster['items']),cluster['region'],cluster['center'][0],cluster['center'][1]))
    groups=[]
    for number,cluster in enumerate(qualified,1):
        group_id=f'G{number:02d}'
        name=f'Location group {number} / 位置群組 {number}'
        members=[item[0] for item in cluster['items']]
        points=[item[1] for item in cluster['items']]
        timing_ids=sorted({event.get('pattern_id') for event in members if event.get('pattern_id')})
        timing_regular=sum(bool(event.get('pattern_id')) for event in members)
        representative=max(members,key=lambda event:event.get('peak_score',0))
        group=dict(id=group_id,name=name,region=cluster['region'],occurrences=len(members),
                   event_ids=[event['id'] for event in members],representative_event_id=representative['id'],
                   first_seconds=round(min(event['peak_seconds'] for event in members),3),
                   last_seconds=round(max(event['peak_seconds'] for event in members),3),
                   center_source_pixels=[round(float(np.median([p[0] for p in points])),1),
                                         round(float(np.median([p[1] for p in points])),1)],
                   grouping_radius_pixels=round(radius,1),timing_pattern_ids=timing_ids,
                   timing_regular_events=timing_regular,
                   timing_unclassified_events=len(members)-timing_regular,
                   review_hint='Repeated at a similar source-image location. Timing may be stable, changing, or irregular; human review required.')
        groups.append(group)
        for event in members:
            event['location_group_id']=group_id
            event['location_group_name']=name
    return groups

def classify_visual_behavior(events, location_groups, metadata):
    """Conservatively flag compact repeated flicker and downward motion.

    Labels are review aids only. A fixed drip outlet can resemble reflection
    jitter, and older results do not contain within-event motion endpoints.
    """
    ...
    def transformed_center(box,transform):
        x,y=center(box)
        raw=np.array([[x,y,1]],np.float32)@np.array(transform,np.float32).T
        return (float(raw[0,0]),float(raw[0,1]))
    for event in events:
        event['visual_classification']='uncertain'
        event['motion_dx_pixels']=None
        event['motion_dy_pixels']=None
        first=event.get('first_source_bbox');last=event.get('last_source_bbox')
        if first and last:a,b=center(first),center(last)
        elif event.get('first_bbox') and event.get('last_bbox'):
            a=transformed_center(event['first_bbox'],event.get('first_transform',[[1,0,0],[0,1,0]]))
            b=transformed_center(event['last_bbox'],event.get('last_transform',[[1,0,0],[0,1,0]]))
        else:continue
        dx,dy=b[0]-a[0],b[1]-a[1]
        event['motion_dx_pixels']=round(dx,2);event['motion_dy_pixels']=round(dy,2)
        if (event.get('positive_frames',1)>=2 and dy>=max(6.0,height*.0055) and
                dy>=abs(dx)*.70):
            event['visual_classification']='possible_moving_droplet'
    for group in location_groups:
        members=[by_id[event_id] for event_id in group.get('event_ids',[]) if event_id in by_id]
        points=[]
        for event in members:
            box=event.get('peak_source_bbox')
            if box:points.append(center(box))
        if points:
            array=np.array(points,np.float64);median=np.median(array,axis=0)
            distances=np.sqrt(((array-median)**2).sum(axis=1))
            p90=float(np.percentile(distances,90))
        else:p90=999.0
        span=max(.001,group.get('last_seconds',0)-group.get('first_seconds',0))
        per_minute=60*len(members)/max(1.0,span)
        brief_fraction=(sum(event.get('positive_frames',1)<=3 for event in members)/max(1,len(members)))
        compact=max(0.0,min(1.0,1-p90/20.0))
        recurrence=min(1.0,len(members)/20.0)
        density=min(1.0,per_minute/10.0)
        score=.35*compact+.25*recurrence+.20*density+.20*brief_fraction
        moving=sum(event.get('visual_classification')=='possible_moving_droplet' for event in members)
        likely_reflection=(len(members)>=8 and p90<=12 and brief_fraction>=.65 and score>=.65)
        if likely_reflection:classification='likely_reflection_jitter'
        elif moving>=max(2,math.ceil(len(members)*.15)):classification='possible_moving_droplet'
        else:classification='uncertain'
        group.update(visual_classification=classification,reflection_jitter_score=round(score,3),
                     position_spread_p90_pixels=round(p90,2),events_per_minute=round(per_minute,2),
                     brief_event_fraction=round(brief_fraction,3),moving_event_count=moving,
                     visual_review_hint=('Compact, frequent, short-lived activity at one location; likely reflection jitter, but a fixed drip source can look similar.'
                                         if classification=='likely_reflection_jitter' else
                                         'Downward within-event motion was observed; possible moving droplet, but visual review is required.'
                                         if classification=='possible_moving_droplet' else
                                         'Evidence is insufficient to distinguish reflection jitter from fluid or another process change.'))
        for event in members:
            if event.get('visual_classification')!='possible_moving_droplet' and likely_reflection:
                event['visual_classification']='likely_reflection_jitter'
...
def write_csv(folder, events):
    fields=['id','region','kind','start_seconds','peak_seconds','end_seconds',
            'location_group_id','location_group_name','visual_classification','motion_dx_pixels','motion_dy_pixels',
            'pattern_id','pattern_class','pattern_interval_seconds','pattern_frequency_trend',
            'status','confirmed_drips','positive_frames','max_components','peak_score',
            'export_playback_speed','export_fps','export_duration_seconds','available_export_speeds',
            'clip','closeup_clip','notes']
...
def write_patterns_csv(folder, patterns):
    fields=['id','classification','region','occurrences','first_seconds','last_seconds',
            'interval_seconds','interval_start_seconds','interval_end_seconds',
            'frequency_per_minute','frequency_trend','regularity_score',
...
def write_location_groups_csv(folder, groups):
    fields=['id','name','region','occurrences','first_seconds','last_seconds',
            'center_source_pixels','grouping_radius_pixels','timing_pattern_ids',
            'timing_regular_events','timing_unclassified_events','visual_classification',
            'reflection_jitter_score','position_spread_p90_pixels','events_per_minute',
            'brief_event_fraction','moving_event_count','representative_event_id',
            'event_ids','review_hint','visual_review_hint']
    rows=[]
    for group in groups:
        row=dict(group)
        row['center_source_pixels']=';'.join(str(value) for value in group.get('center_source_pixels',[]))
        row['timing_pattern_ids']=';'.join(group.get('timing_pattern_ids',[]))
        row['event_ids']=';'.join(group.get('event_ids',[]))
        rows.append(row)
    with (Path(folder)/'location_groups.csv').open('w',newline='',encoding='utf-8-sig') as file:
        writer=csv.DictWriter(file,fieldnames=fields,extrasaction='ignore')
        writer.writeheader()
        writer.writerows(rows)
...
def export_event(cap, metadata, event, box, folder, config, cancel, timeline=None, pause_gate=None):
    fps=metadata['fps']
    playback_speed=float(config.get('export_playback_speed',1.0))
    if not .05<=playback_speed<=2.0:raise ValueError('Export playback speed must be from 0.05x to 2.0x.')
    output_fps=max(.5,fps*playback_speed)
    ...
    speed_text=f'{playback_speed:g}'
    speed_tag='' if abs(playback_speed-1)<1e-9 else '_'+speed_text.replace('.','p')+'x'
    stem=f"{event['id']}_{event['region'].lower()}_{event['peak_seconds']:.3f}s{speed_tag}"
    ...
    try:
        if config['export_clips']:
            ...
            full=create_writer(full_path,output_fps,(fw,fh))
            close=create_writer(crop_path,output_fps,(cw*2,ch*2))
    ...
    event['export_playback_speed']=playback_speed
    event['export_fps']=round(output_fps,4)
    event['export_duration_seconds']=round((end-start)/output_fps,4)
    if config['export_clips']:
        record=dict(playback_speed=playback_speed,output_fps=round(output_fps,4),
                    duration_seconds=event['export_duration_seconds'],clip=event['clip'],
                    closeup_clip=event['closeup_clip'],snapshot=event['snapshot'])
        exports=[item for item in event.get('exports',[]) if abs(float(item.get('playback_speed',1))-playback_speed)>1e-9]
        exports.append(record)
        event['exports']=sorted(exports,key=lambda item:float(item['playback_speed']))
        event['available_export_speeds']=';'.join(f"{float(item['playback_speed']):g}x" for item in event['exports'])
...
def analyze(path, output_root=None, config=None, progress=None, cancel=None, pause_gate=None, partial=None):
    ...
    try:
        ...
        def emit_partial(scanned_frame):
            ...
            location_groups=classify_location_groups(events,metadata)
            classify_visual_behavior(events,location_groups,metadata)
            snapshot=json.loads(json.dumps(report))
            elapsed=max(.001,time.perf_counter()-start_time)
            snapshot.update(state='paused_preview',events=events,patterns=patterns,location_groups=location_groups,
        ...
        report['location_groups']=classify_location_groups(events,metadata)
        classify_visual_behavior(events,report['location_groups'],metadata)

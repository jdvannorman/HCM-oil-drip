...
def strip_present(frame,edges,debug=False):
    """Cheap guard that verifies material still exists at tracked boundaries."""
    height,width=frame.shape[:2]
    small=cv2.resize(frame,(640,round(height*640/width)))
    h,w=small.shape[:2]
    gray=cv2.cvtColor(small,cv2.COLOR_BGR2GRAY).astype(np.float32)
    b,_,r=cv2.split(small.astype(np.float32))
    blue=cv2.GaussianBlur(b-r,(7,7),0)
    texture=np.sqrt(np.maximum(0,cv2.blur(gray*gray,(11,5))-cv2.blur(gray,(11,5))**2))
    yy=np.linspace(int(h*.67)+4,int(h*.98)-4,45).astype(int)
    metrics=[]
    for side,direction in [('left',1),('right',-1)]:
        a,b0=edges[side]
        xx=np.rint((a*(yy/h)+b0)*w).astype(int)
        if xx.min()<18 or xx.max()>=w-18:return False
        inside=xx+direction*12
        outside=xx-direction*12
        delta=blue[yy,inside]-blue[yy,outside]
        texture_delta=texture[yy,outside]-texture[yy,inside]
        contrast=float(np.median(delta))
        support=float(np.mean((delta>5)|(texture_delta>7)))
        metrics.append((contrast,support))
    present=all(contrast>12 and support>.72 for contrast,support in metrics)
    if debug:print('presence',metrics,present)
    return present

DEFAULT_STRIP_POLYGONS={
    'Left': [[0,.72],[1/3,.72],[1/3,.98],[0,.98]],
    'Center': [[1/3,.72],[2/3,.72],[2/3,.98],[1/3,.98]],
    'Right': [[2/3,.72],[1,.72],[1,.98],[2/3,.98]],
}
...
class AreaTracker:
    """Adaptive edge checks: frequent at startup, sparse after geometry settles."""
    def __init__(self,fps,config=None):
        self.fps=fps
        config=config or {}
        self.startup_frames=max(1,round(fps*float(config.get('roi_startup_seconds',5))))
        self.startup_stride=max(1,int(config.get('roi_startup_stride',5)))
        self.stable_interval=max(1,round(fps*float(config.get('roi_stable_recheck_seconds',600))))
        self.camera_interval=max(1,round(fps*float(config.get('roi_camera_check_seconds',1))))
        self.presence_interval=max(1,round(fps*float(config.get('roi_presence_check_seconds',1))))
        self.last=None
        self.last_good=None
        self.pending=None
        ...
        self.stable_hits=0
        self.settled=False
        self.material_absent=False
        self.material_hits=0
    ...
    def update(self,frame,index):
        if self.last_index is not None and index<=self.last_index:
            ...
            self.camera_ref=None; self.camera_points=None; self.camera_shift=np.zeros(2,np.float32)
            self.next_full=0; self.next_camera=0; self.recovering=False
            self.stable_hits=0; self.settled=False
            self.material_absent=False; self.material_hits=0; self.next_presence=0
        self.last_index=index
        prechecked=None
        if self.material_absent:
            if index<self.next_presence:
                status='material_confirming' if self.material_hits else 'no_material'
                return dict(status=status,edges=None,geometry_check='material_absent')
            self.next_presence=index+self.presence_interval
            self.presence_checks+=1
            prechecked=locate_strip(frame)
            self.full_checks+=1
            if not prechecked:
                self.material_hits=0
                return dict(status='no_material',edges=None,geometry_check='material_absent')
            self.material_hits+=1
            if self.material_hits<2:
                return dict(status='material_confirming',edges=None,geometry_check='material_found')
            self.material_absent=False
            self.material_hits=0
            self.last=None;self.last_good=None;self.pending=None;self.pending_count=0
            self.recovering=False
        elif self.last is not None and index>=self.next_presence:
            self.next_presence=index+self.presence_interval
            self.presence_checks+=1
            if not strip_present(frame,self.last):
                # A real width/position change can fail the cheap check too.
                # A full location on this one frame distinguishes changed
                # material from an empty machine view.
                prechecked=locate_strip(frame)
                self.full_checks+=1
                if not prechecked:
                    self.material_absent=True
                    self.material_hits=0
                    self.recovering=False
                    return dict(status='no_material',edges=None,geometry_check='material_absent')
        camera_due=self.last is None or index>=self.next_camera
        camera_shift=self.camera_shift.tolist()
        force_full=False
        if camera_due:
            previous=np.array(camera_shift,np.float32)
            camera_shift=self.estimate_camera_shift(frame)
            self.camera_checks+=1
            self.next_camera=index+self.camera_interval
            delta=np.array(camera_shift)-previous
            if self.last is not None:
                # Edges are in image coordinates, so follow small horizontal
                # camera translations between the less frequent full checks.
                for side in ('left','right'):self.last[side][1]+=float(delta[0])
                self.last['camera_shift']=camera_shift
                if np.max(np.abs(delta))>.004:force_full=True
        in_startup=index<self.startup_frames
        full_due=prechecked is not None or self.last is None or self.recovering or force_full or index>=self.next_full
        if not full_due:
            self.last_good=index
            return dict(status='tracking',edges=copy.deepcopy(self.last),geometry_check='predicted')
        found=prechecked if prechecked is not None else locate_strip(frame)
        if prechecked is None:self.full_checks+=1
        ...
        if found:
            ...
            # The edge estimator naturally jitters a few pixels on reflections;
            # treat changes below ~0.8% of image width as stable.
            if self.last is not None and jump<.008:self.stable_hits+=1
            else:self.stable_hits=0
            if self.stable_hits>=3:self.settled=True
            if jump>.025:
                self.settled=False; self.stable_hits=0
                ...
                self.pending=found
                if self.pending_count<3:found=None; self.recovering=True; self.next_full=index+1
                else:self.last=None
            else:
                self.pending=None; self.pending_count=0
                if jump>.012:
                    self.settled=False; self.stable_hits=0
                    self.next_full=index+self.startup_stride
        if found:
            if self.last:
            ...
            self.last_good=index
            self.recovering=False
            self.next_presence=max(self.next_presence,index+self.presence_interval)
            if self.next_full<=index:
                # Once several startup checks agree, a once-per-second guard is
                # enough for the remainder of startup; long-term uses the
                # user-selected 1/5/10 minute interval.
                interval=(self.camera_interval if self.settled else self.startup_stride) if in_startup else self.stable_interval
                self.next_full=index+interval
            return dict(status='tracking',edges=copy.deepcopy(found),geometry_check='full')
        if self.last is not None and (index-self.last_good)/self.fps<=.5:
            self.recovering=True
            self.next_full=index+1
            return dict(status='held',edges=copy.deepcopy(self.last),geometry_check='failed')
        ...
        self.recovering=True
        self.next_full=index+1
        return dict(status='lost',edges=None,geometry_check='failed')

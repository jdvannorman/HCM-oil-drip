...
def analyze_auto(path,output_root,config,progress,cancel,pause_gate=None,partial=None):
    ...
    try:
        ...
        # Search through the video at one-second steps so a recording may begin
        # before the coil enters the camera. No defect detection occurs here.
        calibration_stride=max(1,round(meta['fps']))
        calibration_first_material=None
        for index in range(0,meta['frames'],calibration_stride):
            ...
            if calibration_first_material is None:calibration_first_material=index
            regions=regions_from_edges(state['edges'],config['regions'])
            for r in regions:samples[r['name']].append(gray_blur(rectify(frame,r)[0]))
            if all(len(v)>=3 for v in samples.values()):break
        if not all(len(v)>=3 for v in samples.values()):
            raise ValueError('No reliable coil/strip interval was found in this video. No automatic defect count was produced.')
        ...
        def emit_partial(scanned_frame):
            ...
            classify_visual_behavior(events,location_groups,meta)
            snapshot=copy.deepcopy(report)
        ...
        report['tracking']=dict(frame_counts=counts,geometry_checks=check_counts,
                                ...
                                material_presence_checks=tracker.presence_checks,
                                first_calibration_material_frame=calibration_first_material,
                                usable_frames=counts.get('tracking',0),total_frames=len(timeline))
        skipped=len(timeline)-counts.get('tracking',0)
        no_material=counts.get('no_material',0)+counts.get('material_confirming',0)
        report['warnings']=[f'{skipped} frames not analyzed while material was absent/unconfirmed or areas were held, lost, or recalibrating. {no_material} frames were gated by material presence. Zero events is not proof of no dripping.',
